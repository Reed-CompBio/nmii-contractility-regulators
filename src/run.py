...
#
# Updated Jan 2020, Dec 2021, Aug 2023 by Anna Ritz

from __future__ import print_function
from datetime import datetime
import sys

#OUTPUT_PREFIX= output_dir+"/collapsed_"

def main(input_dir,output_dir): # EEK, KT added comments to this
    global OUTPUT_PREFIX
    OUTPUT_PREFIX= output_dir+"/collapsed_"

    #Input files
    INTERACTOME_FILE = input_dir+"/interactome-flybase-collapsed-evidence.txt" #interactome file
    POSITIVES_FILE = input_dir+"/positive-ids.txt" #Positive node file
    NODE_COLS = [3,4]

    print('Start: ' + str(datetime.now()))
    #read interactome and positive node files
    edges, nodes = read_edge_file(INTERACTOME_FILE,NODE_COLS)
...
def read_edge_file(filename,node_cols): ##taken from L.T.'s code and then edited by K.T (labtime), commented by EEK
    nodes = set()
    edges = set()
...
if __name__ == '__main__':
    if len(sys.argv) != 3:
        sys.exit('USAGE: python run.py <INPUT_DIR> <OUTPUT_DIR>')
